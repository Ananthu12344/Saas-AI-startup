...
import { createOpenAIAdapterForCredential } from "@/lib/providers/openai-worker"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: provider } = await adminClient
      ...
      .eq("slug", "openai")
      .single()
    if (!provider) return Response.json({ error: "OpenAI credential required" }, { status: 400 })
    const adapter = await createOpenAIAdapterForCredential(adminClient, credential.id)
    ...
    return Response.json({ error: "OpenAI usage sync failed" }, { status: 502 })
  }
}
