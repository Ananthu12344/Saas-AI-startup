...
The OpenAI completions usage adapter now targets `GET /v1/organization/usage/completions`, requests `group_by=model`, converts Unix-second buckets into normalized events, and rejects missing models, required counts, or unsupported non-zero token dimensions. The endpoint contract is documented in the [OpenAI Usage API reference](https://platform.openai.com/docs/api-reference/usage/completions). It requires an organization-level key with access to the Usage API. `lib/providers/crypto.ts` defines the new-entry format as versioned AES-256-GCM ciphertext (`v1.<key-version>.<iv>.<tag>.<ciphertext>`), configured only with protected server runtime variables `CLARITY_CREDENTIAL_MASTER_KEY` (base64-encoded 32-byte key) and `CLARITY_CREDENTIAL_KEY_VERSION`. Existing ciphertext without this documented format must be rejected and re-entered through a server-only writer; neither variable belongs in `NEXT_PUBLIC_*`, logs, browser code, or metadata.

Credential entry is exposed only through the authenticated `POST /api/credentials/[credentialId]/secret` route. It requires an owner/admin session, validates membership through the session client, and writes encrypted material with the protected service-role client. Configure `SUPABASE_SERVICE_ROLE_KEY` only in the server runtime. Submit the provider key over HTTPS as `{ "secret": "…" }`; the route returns only `{ "ok": true }` and never echoes the key.
